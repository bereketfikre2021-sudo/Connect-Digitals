...
export default function ServiceCard({icon, title, desc, details}){
  ...
  return (
    <article className="bg-white rounded-2xl p-4 sm:p-6 shadow hover:shadow-lg transition-all duration-300 hover:-translate-y-1 h-full flex flex-col">
      <div className="text-3xl sm:text-4xl mb-3 sm:mb-4 text-center" aria-hidden="true">{icon}</div>
      <h3 className="text-base sm:text-lg font-display font-semibold text-primaryNavy mb-2 sm:mb-3">{title}</h3>
      <p className="text-gray-600 text-sm leading-relaxed mb-4 font-sans flex-grow">{desc}</p>
      
      {isExpanded && (
        <div className="mb-4 p-4 bg-gray-50 rounded-xl" role="region" aria-label={`Additional details for ${title}`}>
      ...
      <button
        ...
        aria-label={`${isExpanded ? 'Hide' : 'Show'} additional details for ${title}`}
      >
